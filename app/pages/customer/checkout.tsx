...
interface Quote { billingInterval: 'month' | 'year'; currency: string; discountMinor: number; intervalCount: number; packageName: string; subtotalMinor: number; taxMinor: number; token: string; totalMinor: number; trialEnabled: boolean }
...
export default function CheckoutPage() {
	...
	async function purchase(startTrial: boolean): Promise<void> {
		if (!quote) return;
		setSubmitting(true); setError('');
		try {
			const response = await authenticatedFetch('/api/v1/checkouts', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ quoteToken: quote.token, startTrial }) });
			const body = await response.json() as { data?: { nextUrl?: string }; message: string; status: boolean };
			if (!response.ok || !body.status || !body.data?.nextUrl) throw new Error(body.message);
			navigate(body.data.nextUrl);
	...
	return <main className="min-h-screen bg-app-canvas px-5 py-10 text-app-text sm:px-8"><div className="mx-auto max-w-5xl"><Link className="inline-flex items-center gap-2 text-sm font-semibold text-app-muted" to="/#plans"><ArrowLeft className="size-4" /> Plans</Link><div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]"><section className="rounded-[2rem] border border-brand-primary/10 bg-app-surface p-6 sm:p-9"><p className="text-sm font-semibold text-brand-primary dark:text-brand-action">Checkout · {packageSlug}</p><h1 className="mt-2 text-4xl font-black">Confirm your purchase</h1><p className="mt-4 leading-7 text-app-muted">{quote.trialEnabled ? 'Start the included no-card trial, or pay now.' : 'Continue to secure payment.'} Workspace setup begins only after the selected path is verified.</p>{error && <p className="mt-6 rounded-xl bg-rose-500/10 p-3 text-sm text-rose-600 dark:text-rose-300">{error}</p>}<div className={`mt-8 grid gap-3 ${quote.trialEnabled ? 'sm:grid-cols-2' : ''}`}>{quote.trialEnabled && <button className="rounded-2xl border border-brand-primary/20 px-5 py-4 font-bold disabled:opacity-60" disabled={submitting} onClick={() => void purchase(true)} type="button">Start free trial</button>}<button className="inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-action px-5 py-4 font-bold text-brand-ink disabled:opacity-60" disabled={submitting} onClick={() => void purchase(false)} type="button">{submitting ? <LoaderCircle className="size-4 animate-spin" /> : <>Pay now <ArrowRight className="size-4" /></>}</button></div><p className="mt-4 text-xs leading-5 text-app-muted">Payments are verified by the server before workspace provisioning begins.</p></section><aside className="h-fit rounded-[2rem] bg-brand-primary p-6 text-white sm:p-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-brand-action">Order summary</p><h2 className="mt-5 text-3xl font-black">{quote.packageName}</h2><p className="mt-2 text-white/60">{term} billing</p><dl className="mt-8 space-y-4 border-t border-white/10 pt-6 text-sm"><div className="flex justify-between"><dt className="text-white/60">Package</dt><dd>{money(quote.subtotalMinor)}</dd></div>{quote.discountMinor > 0 && <div className="flex justify-between text-brand-action"><dt>Discount</dt><dd>-{money(quote.discountMinor)}</dd></div>}<div className="flex justify-between"><dt className="text-white/60">GST</dt><dd>{money(quote.taxMinor)}</dd></div><div className="flex justify-between border-t border-white/10 pt-4 text-lg font-bold"><dt>Total</dt><dd>{money(quote.totalMinor)}</dd></div></dl></aside></div></div></main>;
}
