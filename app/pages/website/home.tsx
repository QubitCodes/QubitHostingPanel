...
export function LandingPage({ catalogue }: { catalogue: CataloguePackage[] }) {
	...
	useEffect(() => {
		const timeout = window.setTimeout(() => {
			...
			setHasSession(Boolean(sessionStorage.getItem('accessToken')));
		}, 0);
	...
	return <main className="min-h-screen overflow-hidden bg-app-canvas text-app-text">
		...
		<section className="relative" id="top">
			...
			<div className="relative mx-auto grid max-w-[90rem] gap-12 px-5 py-20 sm:px-8 sm:py-28 lg:grid-cols-[1.08fr_.92fr] lg:px-12 lg:py-36">
				<div className="max-w-4xl"><p className="inline-flex items-center gap-2 rounded-full border border-brand-action/30 bg-brand-action/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[.16em] text-brand-primary dark:text-brand-action"><span className="size-1.5 rounded-full bg-brand-action" /> Built for serious small teams</p><h1 className="mt-8 text-5xl font-black leading-[.94] tracking-[-.065em] sm:text-7xl lg:text-[6.6rem]">Hosting that feels<br /><span className="text-brand-primary dark:text-brand-action">under control.</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-app-muted sm:text-xl">Deploy applications, keep commercial limits honest, and see the state of your operation without assembling another fragile stack of dashboards.</p><div className="mt-10 flex flex-col gap-3 sm:flex-row"><a className="inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-action px-6 py-4 font-bold text-brand-ink" href="#plans">Explore plans <ArrowRight className="size-4" /></a><Link className="inline-flex items-center justify-center gap-2 rounded-2xl border border-brand-primary/20 bg-app-surface px-6 py-4 font-bold text-brand-primary dark:text-brand-action" to={hasSession ? '/admin/overview' : '/login'}>{hasSession ? 'Continue to your panel' : 'Already have an account?'} <ChevronRight className="size-4" /></Link></div><div className="mt-12 flex flex-wrap gap-x-7 gap-y-3 text-sm text-app-muted">{['WhatsApp OTP', 'AWS-backed infrastructure', 'Clear plan limits'].map((item) => <span className="flex items-center gap-2" key={item}><Check className="size-4 text-brand-action" />{item}</span>)}</div></div>
		...
		<section className="px-5 pb-8 sm:px-8 lg:px-12"><div className="mx-auto max-w-[90rem] overflow-hidden rounded-[2rem] bg-brand-action px-6 py-14 text-brand-ink sm:px-10 lg:flex lg:items-center lg:justify-between lg:px-16 lg:py-16"><div><p className="text-xs font-black uppercase tracking-[.2em]">Ready when you are</p><h2 className="mt-3 max-w-3xl text-4xl font-black tracking-[-.045em] sm:text-5xl">Give your next deployment a proper home.</h2></div><div className="mt-8 flex shrink-0 flex-col gap-3 sm:flex-row lg:mt-0"><a className="rounded-2xl bg-brand-primary px-6 py-4 text-center font-bold text-white" href="#plans">Compare plans</a><Link className="rounded-2xl border border-brand-ink/20 px-6 py-4 text-center font-bold" to={hasSession ? '/admin/overview' : '/login'}>{hasSession ? 'Open panel' : 'Sign in'}</Link></div></div></section>
