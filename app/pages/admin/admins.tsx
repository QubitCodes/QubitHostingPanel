...
export default function AdminsPage() {
	...
	function closePanel(): void { navigate('/admin/administrators'); setDetail(undefined); }
	...
	async function createAdmin(values: CreateAdminForm): Promise<void> {
		...
		try {
			...
			toast.success('Administrator created.'); form.reset(); await loadBase(); navigate(`/admin/administrators/${created.id}`);
	...
	return <div className="mx-auto max-w-7xl"><div>
		<div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-teal-700 dark:text-[#e0ff71]">Platform access</p><h2 className="mt-1 text-3xl font-bold tracking-tight">Administrators</h2><p className="mt-2 text-sm text-stone-600 dark:text-stone-300">Manage passwordless identities, roles, overrides, sessions, and security activity.</p></div><Link className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#123c32] px-4 py-2.5 text-sm font-semibold text-white dark:bg-[#e0ff71] dark:text-[#123c32]" to="/admin/administrators/create"><Plus className="size-4" />Add administrator</Link></div>
		<div className="mt-8 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm dark:border-stone-800 dark:bg-[#1b211e]"><div className="overflow-x-auto"><table className="min-w-full divide-y divide-stone-200 text-sm dark:divide-stone-800"><thead className="bg-stone-50 text-left text-xs uppercase tracking-wide text-stone-500 dark:bg-stone-950/50"><tr><th className="px-5 py-3">Administrator</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Verified</th><th className="px-5 py-3"><span className="sr-only">Open</span></th></tr></thead><tbody className="divide-y divide-stone-200 dark:divide-stone-800">{admins.map((admin) => <tr key={admin.id}><td className="px-5 py-4"><p className="font-semibold">{admin.displayName || 'Unnamed administrator'}</p><p className="text-stone-500">{admin.mobileE164}</p></td><td className="px-5 py-4 capitalize">{admin.status}</td><td className="px-5 py-4">{admin.mobileVerifiedAt ? 'Yes' : 'Pending first login'}</td><td className="px-5 py-4 text-right"><Link className="font-semibold text-teal-700 hover:underline dark:text-[#e0ff71]" to={`/admin/administrators/${admin.id}`}>Manage</Link></td></tr>)}</tbody></table></div></div>
	</div>
