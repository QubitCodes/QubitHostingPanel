...
export default function PackagesPage() {
	...
	const [twoYearAmount, setTwoYearAmount] = useState('');
	const [threeYearAmount, setThreeYearAmount] = useState('');
	...
	useEffect(() => {
		...
		void api<PackageRecord>(`/api/v1/packages/${packageSlug}`)
			.then((record) => {
				...
				const yearly = record.prices?.find((price) => price.isActive && price.billingInterval === 'year' && price.intervalCount === 1);
				const twoYear = record.prices?.find((price) => price.isActive && price.billingInterval === 'year' && price.intervalCount === 2);
				const threeYear = record.prices?.find((price) => price.isActive && price.billingInterval === 'year' && price.intervalCount === 3);
				setMonthlyAmount(monthly ? String(monthly.amountMinor / 100) : '');
				setYearlyAmount(yearly ? String(yearly.amountMinor / 100) : '');
				setTwoYearAmount(twoYear ? String(twoYear.amountMinor / 100) : '');
				setThreeYearAmount(threeYear ? String(threeYear.amountMinor / 100) : '');
	...
	async function savePrices(): Promise<void> {
		...
		try {
			await api(`/api/v1/packages/${packageSlug}/prices`, {
				...
				body: JSON.stringify({ currency: 'INR', monthlyAmount: Number(monthlyAmount), yearlyAmount: Number(yearlyAmount), twoYearAmount: Number(twoYearAmount), threeYearAmount: Number(threeYearAmount), taxBehavior: 'exclusive', isPublic: pricePublic }),
			});
