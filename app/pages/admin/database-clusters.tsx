...
interface Cluster {
	...
	managementTlsMode: 'disabled' | 'require' | 'verify-full';
...
export default function DatabaseClustersPage() {
	...
	async function updateCluster(event: FormEvent<HTMLFormElement>): Promise<void> {
		...
		const managementHost = String(form.get('managementHost') ?? '').trim(); const managementPort = String(form.get('managementPort') ?? '').trim();
		try { await api<Cluster>(`/api/v1/operations/database-clusters/${cluster.code}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ maximumDatabases: Number(form.get('maximumDatabases')), status: form.get('status'), managementHost: managementHost || null, managementPort: managementPort ? Number(managementPort) : null, managementTlsMode: form.get('managementTlsMode') }) }); toast.success('Cluster settings updated.'); await load(); }
