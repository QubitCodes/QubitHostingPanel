...
import { logicalDatabasePublicIdSchema, moveLogicalDatabaseSchema } from '@schemas/logicalDatabase';
...
export async function action({ params, request }: { params: { databaseId?: string; workspaceId?: string }; request: Request }): Promise<Response> {
	...
	const input = await parseJson(request, moveLogicalDatabaseSchema);
	return input instanceof Response ? input : DatabaseLifecycleController.move(request, workspaceId.data, databaseId.data, input, getRequestMetadata(request));
}
