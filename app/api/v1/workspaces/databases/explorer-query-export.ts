...
import { databaseQueryExportSchema } from '@schemas/databaseQuery';
import { logicalDatabasePublicIdSchema } from '@schemas/logicalDatabase';
import { workspacePublicIdSchema } from '@schemas/workspace';
import { getRequestMetadata, parseJson } from '@utils/request';

/** Exports one validated read query as bounded CSV. */
export async function action({ params, request }: { params: { databaseId?: string; workspaceId?: string }; request: Request }): Promise<Response> {
	...
	const input = await parseJson(request, databaseQueryExportSchema);
	return input instanceof Response ? input : DatabaseExplorerController.exportQuery(request, workspaceId.data, databaseId.data, input, getRequestMetadata(request));
}
