...
import { cloneLogicalDatabaseSchema, logicalDatabasePublicIdSchema } from '@schemas/logicalDatabase';
...
export async function action({ params, request }: { params: { databaseId?: string; workspaceId?: string }; request: Request }): Promise<Response> {
	...
	const input = await parseJson(request, cloneLogicalDatabaseSchema);
	return input instanceof Response ? input : DatabaseLifecycleController.clone(request, workspaceId.data, databaseId.data, input, getRequestMetadata(request));
}
