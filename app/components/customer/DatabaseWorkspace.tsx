...
export function DatabaseWorkspace({ credential, onCredentialAction, onTogglePassword, passwordVisible, record, submitting, workspacePublicId }: Props) {
	...
	const activeSection = requestedSection && ['backups', 'connection', 'objects', 'overview', 'settings', 'tables'].includes(requestedSection) ? requestedSection : 'overview';
	...
	return <div className="mx-auto max-w-[100rem]">
		...
		<div className="py-7">
			{activeSection === 'overview' && <div className="grid gap-5"><dl className="grid gap-4 rounded-2xl border border-brand-primary/10 p-5 sm:grid-cols-2 lg:grid-cols-3">{[['Engine', `${record.engine} ${record.engineVersion}`], ['Status', record.status], ['Username', record.username], ['Connected applications', record.connectedApplications.length ? record.connectedApplications.map(({ name }) => name).join(', ') : 'None'], ['Connection limit', record.connectionLimit ?? '—'], ['Storage quota', `${record.storageQuotaMb ?? '—'} MB`], ['Created', new Date(record.createdAt).toLocaleString('en-IN')]].map(([label, value]) => <div key={String(label)}><dt className="text-xs font-bold uppercase text-app-muted">{label}</dt><dd className="mt-1 break-all font-mono text-sm">{String(value)}</dd></div>)}</dl><div className="rounded-2xl border border-brand-primary/10 p-5"><h3 className="font-black">Start exploring</h3><p className="mt-1 text-sm text-app-muted">Browse schemas, open tables and inspect live rows without exposing database credentials to the browser.</p><Link className="mt-4 inline-flex rounded-xl bg-brand-action px-4 py-2 font-bold text-brand-ink" to={`${basePath}/tables`}>Open Tables</Link></div></div>}
			{activeSection === 'tables' && <DatabaseExplorer databaseId={record.id} workspacePublicId={workspacePublicId} />}
			{activeSection === 'objects' && <DatabaseObjects databaseId={record.id} workspacePublicId={workspacePublicId} />}
