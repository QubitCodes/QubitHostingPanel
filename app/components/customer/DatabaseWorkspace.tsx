...
export function DatabaseWorkspace({ credential, onCredentialAction, onTogglePassword, passwordVisible, record, submitting, workspacePublicId }: Props) {
	...
	const activeSection = requestedSection && ['access', 'backups', 'connection', 'objects', 'overview', 'schema', 'settings', 'sql', 'tables', 'transfers'].includes(requestedSection) ? requestedSection : 'overview';
	...
	return <div className="mx-auto max-w-[100rem]">
		...
		<div className="py-7">
			...
			{activeSection === 'access' && <DatabaseAccessManager databaseId={record.id} databaseName={record.databaseName} workspacePublicId={workspacePublicId} />}
