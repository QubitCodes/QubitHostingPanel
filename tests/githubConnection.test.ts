...
import { githubPopupFeatures } from '@root/app/components/applications/deploy-application-form';
...
describe('GitHub connection validation', () => {
	it('centres GitHub setup in a constrained popup window', () => {
		expect(githubPopupFeatures({
			...
			screenX: 100,
			screenY: 20,
		})).toBe('popup=yes,width=720,height=760,left=540,top=140,resizable=yes,scrollbars=yes');
	});
