...
- `/api/v1/workspaces/:workspaceId/databases/:databaseId/explorer/query` - one controlled SQL statement with read-only transactions by default, explicit data-change confirmation, timeout, row cap, and privacy-preserving audit fingerprints.
- `/api/v1/workspaces/:workspaceId/databases/:databaseId/diagnostics` - audited connection, storage, lock, index, and long-running-query diagnostics plus exact-confirmation cancellation without exposing SQL text.
- `/api/v1/workspaces/:workspaceId/databases/:databaseId/import/*` and `/export` - signed short-lived native import staging and direct PostgreSQL/MySQL export streams.
- `/api/v1/workspaces/:workspaceId/databases/:databaseId/access` - list and create reusable read-only, read-write, or custom database grants with optional automatic expiry.
- `/api/v1/workspaces/:workspaceId/databases/:databaseId/access/:grantId` - update or revoke a non-owner grant with exact confirmation and audit evidence.
- `/api/v1/workspaces/:workspaceId/databases/:databaseId/users/:databaseUserId/action` - audited credential reveal/rotation, login suspension/restoration, and dependency-safe user deletion.
...
Database details in the customer dashboard expose table/object inventories, a responsive Schema Designer, a controlled SQL workspace, native import/export, user access grants, live diagnostics, and an **Open DB** action. The action opens `/database/:databaseId/...` in a separately authenticated tab, so multiple databases can be managed concurrently without sharing active-workspace UI state. The Schema Designer manages PostgreSQL schemas plus PostgreSQL/MySQL tables, columns, indexes, primary keys, and foreign keys through validated operations rather than arbitrary SQL. SQL uses read-only transactions by default and exact confirmation for supported data changes. Diagnostics expose database-scoped connections, storage, locks, index signals, and fingerprinted long-running queries; query text is withheld and safe cancellation requires the exact database name. Imports use short-lived signed staging and checksums; direct exports are not retained as backups. Destructive changes require exact confirmation and every attempt is audited. Database users are workspace-owned and reusable within one engine cluster. New users receive generated passwords; existing-user selection never reveals or changes the password. Read-only, read-write, and custom schema/table grants can expire automatically. Password rotation and login suspension apply to the shared user and report the full database impact before execution.
