...
Database details in the customer dashboard expose table/object inventories, a responsive Schema Designer, a controlled SQL workspace, native import/export, user access grants, and an **Open DB** action. The action opens `/database/:databaseId/...` in a separately authenticated tab, so multiple databases can be managed concurrently without sharing active-workspace UI state. The Schema Designer manages PostgreSQL schemas plus PostgreSQL/MySQL tables, columns, indexes, primary keys, and foreign keys through validated operations rather than arbitrary SQL. SQL uses read-only transactions by default and exact confirmation for supported data changes. Imports use short-lived signed staging and checksums; direct exports are not retained as backups. Destructive changes require exact confirmation and every attempt is audited. Database users are workspace-owned and reusable within one engine cluster. New users receive generated passwords; existing-user selection never reveals or changes the password. Read-only, read-write, and custom schema/table grants can expire automatically. Password rotation and login suspension apply to the shared user and report the full database impact before execution.
