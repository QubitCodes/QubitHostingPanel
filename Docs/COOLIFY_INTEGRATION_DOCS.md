...
5. _Note down your Elastic IP (e.g., `203.0.113.50`)._
...
_(Note: Turn off Cloudflare Proxying (Orange Cloud) during initial Coolify setup to allow Let's Encrypt HTTP-01 SSL verification)._
...
_The installation takes ~2-3 minutes. It automatically installs Docker, Docker Compose, Traefik reverse proxy, and bootstraps Coolify containers._
...
4. Configure database settings:
   ...
   - **User:** `postgres`
   - **Password:** _(Generate a strong secure password)_
...
- **Start Command:** `npm run start` _(Runs `react-router-serve build/server/index.js` as defined in your package.json)_
- **Port Exposed:** `3000` _(Default React Router Serve port)_
...
### Step 11.3: Live Ghost Deploy Status Notifications

1. Generate and store a dedicated `COOLIFY_WEBHOOK_SECRET` in the Ghost Deploy application environment.
2. In Coolify, open **Notifications -> Webhook**.
3. Set the notification URL to `https://ghostdeploy.com/api/v1/webhooks/coolify?secret=<COOLIFY_WEBHOOK_SECRET>`.
4. Enable deployment success, deployment failure, and container status notifications.
5. Give the Ghost Deploy Coolify API token `read:sensitive` permission so deployment and runtime logs can be retrieved.
6. Send a test notification, then deploy an application and confirm its authenticated application event stream updates without a page reload.
...
2. **Stripe Usage-Based Metered Billing Integration:**
   ...
   export async function recordTenantStorageUsage(
