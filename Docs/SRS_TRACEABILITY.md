...
| Shared logical databases | Cluster management, PostgreSQL/MySQL allocation, restricted credentials, reveal/rotation, row/schema/advanced-object management, controlled SQL and saved queries, durable native/CSV/JSON transfers, customer views, backups/restores, access grants, automation, diagnostics, safety-backed clone/rename/move, and package-gated external access | Implemented; live destructive lifecycle and host-gateway acceptance pending |
