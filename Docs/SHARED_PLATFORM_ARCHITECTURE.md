...
Initial target images are maintained in GitHub Container Registry as `ghcr.io/qubitcodes/runtime-{node,php,python,ruby,static}`. The approved versions are Node.js 22.23.1 and 24.18.0, PHP 8.3.33 and 8.5.9, Python 3.12.13 and 3.13.14, Ruby 3.4.10, and nginx 1.30.4. Node.js 22.23.1 replaces the requested 22.23.2 because the latter is not an upstream release.
...
The customer dashboard and standalone database manager include a Schema Designer, controlled SQL workspace, editable advanced objects, saved queries, and durable transfer jobs. Schema changes use a closed validated operation model. SQL runs one statement at a time, uses read-only transactions by default, and blocks DDL, privilege, session, filesystem, and multi-statement access; supported data changes require exact database confirmation. Audit metadata stores only operation details and query fingerprints, never SQL text or result values. Imports use short-lived user/workspace/database-bound tokens plus checksums, then run behind a mandatory encrypted safety backup. Native, CSV, and JSON exports are encrypted as short-lived artifacts and remain cancellable/retryable through durable job state.

The database manager also provides live, tenant-scoped diagnostics for database size, table storage, connections, visible locks, index activity, and long-running sessions. Query text is reduced to a one-way fingerprint and statement type before leaving the server. Query cancellation accepts numeric session identifiers only, revalidates database/login ownership inside the engine, requires exact database-name confirmation, and audits both success and failure.

Each logical database has an owner grant plus optional reusable read-only, read-write, or custom users. Custom grants are restricted to the selected PostgreSQL schema/table or MySQL table scope. Grant updates are applied to the engine before local state changes; failed persistence triggers provider rollback. Optional expiries are enforced by the protected background worker. Credential reveal, shared-user rotation, login suspension/restoration, grant revocation, and dependency-safe user deletion require exact confirmation and emit audit events with cross-database impact counts.
...
Clone, rename, and cross-workspace move operations build on the same verified recovery path. Clone reserves database quota before creating a new same-cluster database. Rename restores into the new physical name before panel state switches. Move requires no application bindings, no active transfer, and an isolated login; it reserves target-workspace quota, creates new target-prefixed credentials, moves dependent records, and disables the old external-access rule. Cleanup failures retain the working replacement and return explicit operator-cleanup evidence instead of deleting the only usable copy.
...
- A management endpoint is an operator connection path, not customer-level public database isolation. Customer remote access uses a separate package-gated gateway port and one-to-32-entry IPv4/IPv6 allowlist with optional automatic expiry.
- External-access rules remain pending until the root-owned host agent applies matching listener/firewall state and acknowledges the exact revision. The agent receives container UUIDs and ports, never credentials.
- Workspace credentials are encrypted at rest; a login may be intentionally reused across databases in one workspace, while workspace moves require an isolated login and rotate it.
...
- Customer external database access is disabled by default, IP-allowlisted, expiry-aware, entitlement-controlled, and separately audited. Engine TLS remains dependent on the configured database cluster and public gateway hostname.
