...
The customer dashboard and standalone database manager include a Schema Designer. It permits PostgreSQL schema creation/rename/removal and PostgreSQL/MySQL table, column, index, primary-key, and foreign-key changes through a closed validated operation model. Identifiers are engine-quoted, execution is time-bounded, arbitrary SQL is not accepted, destructive targets require exact confirmation, and audit metadata records only the operation and target rather than SQL or values. A later SQL workspace must use separate read/mutation permissions and must not weaken this boundary.
