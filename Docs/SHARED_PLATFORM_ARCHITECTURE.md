...
The customer dashboard and standalone database manager include a Schema Designer, controlled SQL workspace, and native transfer tools. Schema changes use a closed validated operation model. SQL runs one statement at a time, uses read-only transactions by default, and blocks DDL, privilege, session, filesystem, and multi-statement access; supported data changes require exact database confirmation. Audit metadata stores only operation details and query fingerprints, never SQL text or result values. Native imports use short-lived user/workspace/database-bound tokens plus checksums, while exports stream without creating retained backup records.
