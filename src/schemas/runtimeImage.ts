...
const fields = { code: z.string().trim().regex(/^[a-z0-9][a-z0-9._-]{2,79}$/), defaultPort: z.number().int().min(1).max(65535), digest: z.string().trim().max(255).nullable().optional(), isDefault: z.boolean(), language: z.enum(['static', 'php', 'node', 'python']), metadata: z.record(z.string(), z.unknown()).optional(), registry: z.string().trim().min(1).max(255), repository: z.string().trim().min(1).max(255), status: z.enum(['active', 'deprecated', 'disabled']), tag: z.string().trim().min(1).max(120), version: z.string().trim().min(1).max(40) };
