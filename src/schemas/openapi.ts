...
export const OPENAPI_DOCUMENT = {
	...
	paths: {
		...
		'/workspaces/{workspaceId}/applications': {
			...
			post: { summary: 'Queue a public Git application deployment with non-blocking custom domains', operationId: 'createWorkspaceApplication', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['name', 'runtimeCode', 'repository', 'port'], properties: { name: { type: 'string' }, runtimeCode: { type: 'string' }, repository: { type: 'string', format: 'uri' }, port: { type: 'integer' }, subdomain: { type: 'string' }, domains: { type: 'array', maxItems: 20, items: { type: 'string', format: 'hostname' }, description: 'Custom hostnames stored as pending even when public DNS is not ready.' } } } } } }, responses: { '202': { description: 'Application, platform hostname, pending custom domains, and deployment job persisted.' }, '409': { description: 'Requested domain conflicts.' }, '422': { description: 'Application entitlement exhausted.' } } },
		},
		'/workspaces/{workspaceId}/domains': {
			get: { summary: 'List all workspace domains with connected applications and TLS state', operationId: 'listWorkspaceDomains', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }], responses: { '200': { description: 'Workspace-scoped platform and custom domains.' }, '404': { description: 'Workspace not found.' } } },
			post: { summary: 'Check hostname availability, ownership policy, and current public DNS records', operationId: 'checkWorkspaceDomain', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: false, required: ['hostname'], properties: { hostname: { type: 'string', format: 'hostname' } } } } } }, responses: { '200': { description: 'Availability, owner-approval requirement, and current CNAME, A, and AAAA visibility. Missing DNS does not block application creation.' } } },
		},
		'/workspaces/{workspaceId}/domain-ownership': { get: { summary: 'List owned domain scopes and incoming or outgoing access requests', operationId: 'listWorkspaceDomainOwnership', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }], responses: { '200': { description: 'Ownership scopes and approval queues visible to the workspace member.' }, '404': { description: 'Workspace not found.' } } } },
		'/workspaces/{workspaceId}/domain-access/{requestId}': { post: { summary: 'Approve, reject, or revoke a protected-subdomain access request', operationId: 'respondToDomainAccessRequest', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { name: 'requestId', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: false, required: ['action'], properties: { action: { type: 'string', enum: ['approve', 'reject', 'revoke'] } } } } } }, responses: { '200': { description: 'Owner decision applied and routing synchronized when necessary.' }, '403': { description: 'Only the owning workspace Owner may respond.' }, '422': { description: 'Request state does not allow the requested transition.' } } } },
		...
		'/workspaces/{workspaceId}/applications/{applicationId}/domains': {
			...
			post: { summary: 'Register a custom domain for verification or owner approval', operationId: 'createApplicationDomain', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/ApplicationId' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: false, required: ['hostname'], properties: { hostname: { type: 'string', format: 'hostname' } } } } } }, responses: { '201': { description: 'Domain attached immediately, queued for TXT verification, or submitted to the verified parent-domain owner.' }, '409': { description: 'Hostname is already assigned.' } } },
		},
		'/workspaces/{workspaceId}/applications/{applicationId}/domains/{domainId}': {
			...
			delete: { summary: 'Detach and soft-delete a saved domain', operationId: 'deleteApplicationDomain', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/ApplicationId' }, { $ref: '#/components/parameters/DomainId' }], responses: { '200': { description: 'Domain detached and soft-deleted.' }, '422': { description: 'No verified enabled replacement exists for the platform or primary domain.' } } },
		},
		'/workspaces/{workspaceId}/applications/{applicationId}/domains/{domainId}/verify': { post: { summary: 'Verify DNS ownership and attach a custom domain', operationId: 'verifyApplicationDomain', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/ApplicationId' }, { $ref: '#/components/parameters/DomainId' }], responses: { '200': { description: 'TXT ownership verified, provider synchronized, and TLS provisioning started.' }, '403': { description: 'A different workspace owns the verified parent domain; owner approval is required.' }, '422': { description: 'Verification TXT record was not found.' }, '502': { description: 'Provider synchronization failed without enabling the domain locally.' } } } },
		...
		'/operations/platform-settings': { get: { summary: 'Retrieve platform domain settings', operationId: 'showPlatformSettings', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Configured and effective platform URLs plus ownership-verification policy.' }, '403': { description: 'Permission denied.' } } }, post: { summary: 'Update platform domain settings and ownership-verification policy', operationId: 'updatePlatformSettings', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Platform URL and domain ownership configuration updated.' }, '422': { description: 'Invalid or unsafe configuration.' } } } },
		...
		'/operations/platform-settings/dns-providers': { get: { summary: 'List masked DNS provider connections', operationId: 'listDnsProviderConnections', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Masked connection metadata. Secret tokens are never returned.' }, '403': { description: 'Permission denied.' } } } },
		'/operations/platform-settings/dns-providers/{provider}': { parameters: [{ in: 'path', name: 'provider', required: true, schema: { enum: ['cloudflare', 'godaddy', 'hostinger'], type: 'string' } }], post: { summary: 'Create or rotate an encrypted DNS provider connection', operationId: 'saveDnsProviderConnection', security: [{ bearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { accountIdentifier: { type: ['string', 'null'] }, token: { minLength: 8, type: 'string', writeOnly: true } } } } } }, responses: { '200': { description: 'Connection saved.' }, '422': { description: 'Invalid connection details.' } } }, delete: { summary: 'Remove a DNS provider connection', operationId: 'removeDnsProviderConnection', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Connection removed.' } } } },
		'/operations/platform-settings/dns-providers/{provider}/validate': { parameters: [{ in: 'path', name: 'provider', required: true, schema: { enum: ['cloudflare', 'godaddy', 'hostinger'], type: 'string' } }], post: { summary: 'Validate a stored DNS provider connection', operationId: 'validateDnsProviderConnection', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Provider accepted the credentials.' }, '502': { description: 'Provider rejected or could not validate the credentials.' } } } },
