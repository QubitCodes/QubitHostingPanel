...
export const OPENAPI_DOCUMENT = {
	...
	paths: {
		...
		'/internal/database-gateway/config': {
			get: { summary: 'Retrieve database gateway configuration', operationId: 'getDatabaseGatewayConfig', description: 'Returns active and pending ports, CIDR allowlists, target container UUIDs, and target engine ports without any credentials. Invalid worker secrets receive JSON 404.', parameters: [{ name: 'x-internal-job-secret', in: 'header', required: true, schema: { type: 'string' } }], responses: { '200': { description: 'Deterministic gateway configuration.' }, '404': { description: 'Resource not found.' }, '503': { description: 'Configuration unavailable.' } } },
			post: { summary: 'Acknowledge database gateway reconciliation', operationId: 'acknowledgeDatabaseGatewayConfig', description: 'Activates or fails only rules whose IDs and revisions match the applied host configuration.', parameters: [{ name: 'x-internal-job-secret', in: 'header', required: true, schema: { type: 'string' } }], responses: { '200': { description: 'Matching results recorded.' }, '404': { description: 'Resource not found.' } } },
		},
		...
		'/workspaces/{workspaceId}/databases/{databaseId}/clone': { post: { summary: 'Clone a database into a new logical database', operationId: 'cloneLogicalDatabase', description: 'Reserves target quota, creates a seven-day encrypted safety backup, provisions a new same-engine database using the restricted owner, restores the backup, and audits the result.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '201': { description: 'Database cloned.' }, '409': { description: 'Target name exists.' }, '422': { description: 'Quota or confirmation rejected.' } } } },
		'/workspaces/{workspaceId}/databases/{databaseId}/rename': { post: { summary: 'Safety-backed database rename', operationId: 'renameLogicalDatabase', description: 'Creates and restores a replacement physical database before switching panel state. Exact database and connected-application confirmations are required. Connected applications must be redeployed afterward.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '200': { description: 'Database renamed with cleanup status.' }, '409': { description: 'Target name exists.' }, '422': { description: 'Dependency confirmation rejected.' } } } },
		'/workspaces/{workspaceId}/databases/{databaseId}/move': { post: { summary: 'Move a database to another owned workspace', operationId: 'moveLogicalDatabase', description: 'Requires an unbound database with an isolated login. Target quota is reserved, a safety backup is restored into a newly isolated target-workspace database, ownership records move transactionally, and new credentials are returned once.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '200': { description: 'Database moved and new credentials returned.' }, '409': { description: 'Target name exists.' }, '422': { description: 'Bindings, shared login, active transfer, quota, or confirmation prevents the move.' } } } },
		'/workspaces/{workspaceId}/databases/{databaseId}/external-access': {
			get: { summary: 'Read database external-access policy', operationId: 'getDatabaseExternalAccess', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '200': { description: 'Package availability and sanitized gateway state.' } } },
			put: { summary: 'Create or replace database external-access policy', operationId: 'saveDatabaseExternalAccess', description: 'Package-gated. Accepts one to 32 IPv4/IPv6 CIDRs and optional expiry. The policy remains pending until the root-owned gateway agent acknowledges its matching revision. Cluster management endpoints and credentials are never returned.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '200': { description: 'Existing policy queued.' }, '201': { description: 'New policy queued.' }, '422': { description: 'Package does not allow external access.' } } },
			delete: { summary: 'Disable database external access', operationId: 'disableDatabaseExternalAccess', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '200': { description: 'Policy revoked and queued for gateway removal.' }, '404': { description: 'Policy not found.' } } },
		},
		...
		'/workspaces/{workspaceId}/databases/{databaseId}/explorer/advanced': {
			get: {
				...
				description: 'Returns definitions using the isolated logical-database credential and records an audit event. Strict POST operations create, replace, refresh, or drop supported views, materialized views, sequences, routines, triggers, and events without accepting unrelated server administration statements.',
				security: [{ bearerAuth: [] }],
		...
		'/workspaces/{workspaceId}/databases/{databaseId}/explorer/query/export': {
			post: { summary: 'Export a read query as CSV', operationId: 'exportWorkspaceDatabaseQueryCsv', description: 'Re-runs one read-only statement with the normal timeout and a strict 10,000-row maximum, returns UTF-8 CSV, and audits only the query fingerprint and execution metadata.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: false, required: ['query'], properties: { query: { type: 'string', minLength: 1, maxLength: 100000 }, rowLimit: { type: 'integer', minimum: 1, maximum: 10000, default: 1000 } } } } } }, responses: { '200': { description: 'UTF-8 CSV attachment.' }, '400': { description: 'Strict JSON validation failed.' }, '422': { description: 'The statement is not read-only or execution failed.' } } },
		},
		'/workspaces/{workspaceId}/databases/{databaseId}/saved-queries': {
			get: { summary: 'List personal saved queries', operationId: 'listWorkspaceDatabaseSavedQueries', description: 'Returns only the authenticated member’s encrypted-at-rest saved queries for the selected database. The access is audited.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '200': { description: 'Saved queries retrieved.' }, '404': { description: 'Database not found.' } } },
			post: { summary: 'Create a personal saved query', operationId: 'createWorkspaceDatabaseSavedQuery', description: 'Encrypts SQL before persistence. Query text is excluded from audit metadata.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: false, required: ['name', 'query'], properties: { name: { type: 'string', minLength: 1, maxLength: 120 }, description: { type: 'string', maxLength: 500 }, query: { type: 'string', minLength: 1, maxLength: 100000 }, allowChanges: { type: 'boolean', default: false }, rowLimit: { type: 'integer', minimum: 1, maximum: 500, default: 100 }, isFavorite: { type: 'boolean', default: false } } } } } }, responses: { '201': { description: 'Saved query created.' }, '400': { description: 'Strict JSON validation failed.' }, '422': { description: 'Duplicate name or persistence failure.' } } },
		},
		'/workspaces/{workspaceId}/databases/{databaseId}/saved-queries/{savedQueryId}': {
			patch: { summary: 'Update a personal saved query', operationId: 'updateWorkspaceDatabaseSavedQuery', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }, { in: 'path', name: 'savedQueryId', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', minProperties: 1 } } } }, responses: { '200': { description: 'Saved query updated.' }, '404': { description: 'Saved query not found or not owned by the actor.' } } },
			delete: { summary: 'Delete a personal saved query', operationId: 'deleteWorkspaceDatabaseSavedQuery', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }, { in: 'path', name: 'savedQueryId', required: true, schema: { type: 'string', format: 'uuid' } }], responses: { '200': { description: 'Saved query soft-deleted.' }, '404': { description: 'Saved query not found or not owned by the actor.' } } },
		},
		'/workspaces/{workspaceId}/databases/{databaseId}/diagnostics': {
			get: { summary: 'Inspect live logical-database diagnostics', operationId: 'inspectWorkspaceDatabaseDiagnostics', description: 'Returns database-scoped connection counts, storage, long-running query fingerprints, visible locks, table sizes, and index-usage signals. SQL text and result values are never returned. Every view is audited.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }, { in: 'query', name: 'slowThresholdSeconds', schema: { type: 'integer', minimum: 1, maximum: 300, default: 5 } }], responses: { '200': { description: 'Bounded live diagnostics retrieved.' }, '400': { description: 'Invalid threshold.' }, '404': { description: 'Database not found in the workspace.' }, '502': { description: 'The restricted database endpoint could not supply diagnostics.' } } },
			post: { summary: 'Cancel one active logical-database query', operationId: 'cancelWorkspaceDatabaseQuery', description: 'Cancels only an active session belonging to the selected database and its restricted login. Requires exact database-name confirmation and audits successful and failed attempts.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: false, required: ['confirmation', 'sessionId'], properties: { confirmation: { type: 'string', minLength: 1, maxLength: 255 }, sessionId: { type: 'string', pattern: '^\\d{1,20}$' } } } } } }, responses: { '200': { description: 'Cancellation accepted by the database engine.' }, '400': { description: 'Strict JSON validation failed.' }, '422': { description: 'Confirmation failed or the session is no longer cancellable.' } } },
		},
		...
		'/workspaces/{workspaceId}/databases/{databaseId}/import': { post: { summary: 'Queue a staged database import', operationId: 'importWorkspaceDatabase', description: 'Queues a durable native, CSV, or JSON import. A seven-day safety backup completes before mutation. Progress, retries, cancellation, and failures are retained and audited.', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }, { $ref: '#/components/parameters/DatabaseId' }], responses: { '202': { description: 'Import queued.' }, '400': { description: 'Confirmation or payload validation failed.' }, '422': { description: 'Import cannot be queued.' } } } },
		...
		'/workspaces/{workspaceId}/databases/{databaseId}/transfers': { get: { summary: 'List durable database transfers', operationId: 'listDatabaseTransfers', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Transfer jobs and progress.' } } }, post: { summary: 'Queue a database export', operationId: 'queueDatabaseExport', description: 'Queues a native full-database or bounded CSV/JSON table export. Completed artifacts are encrypted and expire automatically.', security: [{ bearerAuth: [] }], responses: { '202': { description: 'Export queued.' }, '422': { description: 'Export cannot be queued.' } } } },
		'/workspaces/{workspaceId}/databases/{databaseId}/transfers/{jobId}': { post: { summary: 'Cancel or retry a database transfer', operationId: 'actOnDatabaseTransfer', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Action requested.' }, '422': { description: 'Current job state rejects the action.' } } } },
		'/workspaces/{workspaceId}/databases/{databaseId}/transfers/{jobId}/download': { get: { summary: 'Download an encrypted transfer result', operationId: 'downloadDatabaseTransfer', security: [{ bearerAuth: [] }], responses: { '200': { description: 'Decrypted short-lived artifact.' }, '404': { description: 'Artifact missing, expired, or not owned by actor.' } } } },
