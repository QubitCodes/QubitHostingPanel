...
export const OPENAPI_DOCUMENT = {
	...
	paths: {
		...
		'/workspaces/{workspaceId}/domains': {
			...
			post: { summary: 'Check hostname availability and current public DNS records', operationId: 'checkWorkspaceDomain', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/WorkspaceId' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: false, required: ['hostname'], properties: { hostname: { type: 'string', format: 'hostname' } } } } } }, responses: { '200': { description: 'Availability plus current CNAME, A, and AAAA visibility. Missing DNS does not block application creation.' } } },
		},
