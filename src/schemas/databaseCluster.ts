...
export const updateDatabaseClusterSchema = z.object({ maximumDatabases: z.number().int().positive().max(10000).optional(), status: z.enum(['active', 'maintenance', 'unavailable', 'retired']).optional() }).refine((value) => Object.keys(value).length > 0);
