...
export const adminApplicationActionSchema = z.object({ action: z.enum(['start', 'stop', 'restart', 'redeploy', 'suspend', 'unsuspend']), reason: z.string().trim().min(5).max(500) }).strict();
