...
export const logicalDatabaseNameAvailabilitySchema = z.object({ name: z.string().trim().min(2).max(80).regex(/^[a-z0-9]+(?:_[a-z0-9]+)*$/, 'Database name must use lowercase snake_case.') }).strict();
...
export const deleteLogicalDatabaseSchema = z.object({
...
export type DeleteLogicalDatabaseRequest = z.infer<typeof deleteLogicalDatabaseSchema>;
