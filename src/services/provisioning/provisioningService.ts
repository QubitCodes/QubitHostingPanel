...
import { applicationBuilds, applicationDatabaseBindings, applicationDeployments, applicationDomains, customerCheckouts, databaseClusters, logicalDatabases, provisioningJobs, runtimeImages, workspaceResources } from '@db/schema';
		...
		try {
			...
			const deploymentInput = async () => {
				...
				if (configuredApplication) {
					domains = (await db.select({ hostname: applicationDomains.hostname }).from(applicationDomains).where(and(eq(applicationDomains.applicationBuildId, configuredApplication.build.id), eq(applicationDomains.isEnabled, true), eq(applicationDomains.status, 'verified'), isNull(applicationDomains.deletedAt)))).map(({ hostname }) => hostname);
