...
export async function getEffectivePlatformUrls(): Promise<EffectivePlatformUrls> {
	...
	if (!settings) return { applicationBaseDomain: process.env.COOLIFY_WILDCARD_DOMAIN ?? fallbackHost, applicationDomainReady: true, blockedDomainKeywords: [], defaultApplicationSubdomainEnabled: true, panelBaseUrl: fallbackUrl, panelDomainMode: 'same_domain', panelDomainReady: true, publicBaseUrl: fallbackUrl, reservedDomainLabels: ['admin', 'api', 'dashboard', 'panel', 'www'] };
