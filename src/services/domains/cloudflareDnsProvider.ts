...
async function cloudflare<T>(path: string, init?: RequestInit): Promise<T> {
	...
	if (!credential) throw new Error('Cloudflare DNS API token is not configured.');
	const response = await fetch(`https://api.cloudflare.com/client/v4${path}`, { ...init, headers: { authorization: `Bearer ${credential.token}`, 'content-type': 'application/json', ...init?.headers }, signal: AbortSignal.timeout(20_000) });
...
export async function createCloudflareZone(hostname: string): Promise<{ id: string; nameservers: string[] }> {
	const accountId = (await dnsProviderCredential('cloudflare'))?.accountIdentifier;
