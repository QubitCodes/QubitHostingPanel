...
export async function validateDnsProviderCredential(provider: DnsProviderCode): Promise<void> {
	...
	const url = provider === 'cloudflare' ? `https://api.cloudflare.com/client/v4/accounts/${encodeURIComponent(credential.accountIdentifier ?? '')}` : provider === 'godaddy' ? 'https://api.godaddy.com/v1/domains?limit=1' : 'https://developers.hostinger.com/api/domains/v1/portfolio?page=1';
	const response = await fetch(url, { headers: { accept: 'application/json', authorization: `Bearer ${credential.token}` }, signal: AbortSignal.timeout(15_000) });
