...
export class CoolifyHostingProvider implements HostingProvider {
	...
	private async request<T>(path: string, init?: RequestInit): Promise<T> {
		...
		if (!response.ok) {
			...
			const details =
				failure.errors && typeof failure.errors === 'object'
	...
	public async controlApplication(
		applicationId: string,
		action: 'redeploy' | 'restart' | 'start' | 'stop',
	): Promise<{ deploymentId?: string }> {
		const endpoint = action === 'redeploy' ? 'start?force=true' : action;
		const result = await this.request<{ deployment_uuid?: string }>(
			`/applications/${encodeURIComponent(applicationId)}/${endpoint}`,
			{ method: 'POST' },
		);
	...
	public async deleteApplication(applicationId: string): Promise<void> {
		await this.request(
			`/applications/${encodeURIComponent(applicationId)}?delete_configurations=true&delete_volumes=true&docker_cleanup=true&delete_connected_networks=true`,
			{ method: 'DELETE' },
		);
	}
	...
	public async listApplicationDeployments(
		applicationId: string,
		take = 20,
	): Promise<readonly ProviderDeployment[]> {
		const result = await this.request<unknown>(
			`/deployments/applications/${encodeURIComponent(applicationId)}?skip=0&take=${Math.max(1, Math.min(100, Math.trunc(take)))}`,
		);
		const rows = Array.isArray(result)
			? result
			: ((result as { deployments?: unknown; data?: unknown } | null)
					?.deployments ?? (result as { data?: unknown } | null)?.data);
					...
					return {
						...
						commitSha:
							typeof item.commit === 'string'
								? item.commit
								: typeof item.git_commit_sha === 'string'
									? item.git_commit_sha
									: null,
						commitMessage:
							typeof item.commit_message === 'string'
								? item.commit_message
								: null,
						createdAt:
	...
	public async getApplicationState(
		applicationId: string,
	): Promise<{ status: string }> {
		const application = await this.request<CoolifyApplication>(
			`/applications/${encodeURIComponent(applicationId)}`,
		);
		return { status: application.status ?? 'unknown' };
	}

	public async getApplicationDeployment(
		deploymentId: string,
			...
			commitSha:
				typeof item.commit === 'string'
					...
					: typeof item.git_commit_sha === 'string'
						...
						: null,
			commitMessage:
				typeof item.commit_message === 'string' ? item.commit_message : null,
	...
	public async createApplicationScheduledTask(
		applicationId: string,
		input: ProviderScheduledTaskInput,
	): Promise<ProviderScheduledTask> {
		return this.request<ProviderScheduledTask>(
			`/applications/${encodeURIComponent(applicationId)}/scheduled-tasks`,
			{ method: 'POST', body: JSON.stringify(input) },
		);
	}

	public async updateApplicationScheduledTask(
		applicationId: string,
		taskId: string,
		input: ProviderScheduledTaskInput,
	): Promise<ProviderScheduledTask> {
		return this.request<ProviderScheduledTask>(
			`/applications/${encodeURIComponent(applicationId)}/scheduled-tasks/${encodeURIComponent(taskId)}`,
			{ method: 'PATCH', body: JSON.stringify(input) },
		);
	}

	public async deleteApplicationScheduledTask(
		applicationId: string,
		taskId: string,
	): Promise<void> {
		await this.request(
	...
	public async listApplicationScheduledTaskExecutions(
		applicationId: string,
		taskId: string,
	): Promise<readonly ProviderScheduledTaskExecution[]> {
		const result = await this.request<unknown>(
			`/applications/${encodeURIComponent(applicationId)}/scheduled-tasks/${encodeURIComponent(taskId)}/executions`,
		);
		const rows = Array.isArray(result)
			? result
		...
		return Array.isArray(rows)
			? rows.map((row) => {
					...
					return {
						...
						status:
							item.status === 'success' || item.status === 'running'
								? item.status
								: 'failed',
						message:
							typeof item.message === 'string'
								? item.message.slice(0, 10_000)
								: null,
