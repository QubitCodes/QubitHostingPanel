...
const RUNTIME_SEEDS = [
	...
	{ code: 'php-8.3', language: 'php', version: '8.3.33', repository: 'qubitcodes/runtime-php', tag: '8.3.33', defaultPort: 80, isDefault: false, status: 'active' },
	{ code: 'php-8.5', language: 'php', version: '8.5.9', repository: 'qubitcodes/runtime-php', tag: '8.5.9', defaultPort: 80, isDefault: true, status: 'active' },
...
const CRON_ENTITLEMENTS = [
	...
	{ code: 'databases.external_access', name: 'External database access', description: 'Allows IP-restricted database connectivity through the dedicated customer gateway.', valueType: 'boolean' as const, unit: null },
] as const;
...
async function seedPackageCatalogue(): Promise<void> {
	...
	for (const seed of PACKAGE_SEEDS) {
		...
		for (const [code, value] of Object.entries({ 'deployments.manual_enabled': true, 'deployments.auto_enabled': deploymentLimits.auto, 'deployments.history_limit': deploymentLimits.history, 'deployments.retention_days': deploymentLimits.retention, 'applications.custom_system_pages': seed.slug !== 'launch', 'databases.external_access': seed.slug !== 'launch' })) {
			const entitlementId = cronDefinitions.find((item) => item.code === code)?.id;
			if (!entitlementId) throw new Error(`Entitlement ${code} was not seeded.`);
			await db.insert(packageEntitlements).values({ packageId: packageRow.id, entitlementId, booleanValue: typeof value === 'boolean' ? value : null, numericValue: typeof value === 'number' ? value : null }).onConflictDoUpdate({ target: [packageEntitlements.packageId, packageEntitlements.entitlementId], targetWhere: sql`${packageEntitlements.deletedAt} IS NULL`, set: { booleanValue: typeof value === 'boolean' ? value : null, numericValue: typeof value === 'number' ? value : null, updatedAt: new Date() } });
		}
