...
type Action = 'redeploy' | 'restart' | 'start' | 'stop' | 'suspend' | 'unsuspend';
export class AdminApplicationControlController {
	public static async control(request: Request, userPublicId: number, workspacePublicId: number, applicationId: string, input: { action: Action; reason: string }, metadata: RequestMetadata): Promise<Response> {
		const permission = input.action === 'redeploy' ? 'deployments.retry' : input.action === 'unsuspend' ? 'applications.suspend' : `applications.${input.action}`;
		try {
			...
			const providerAction = input.action === 'suspend' ? 'stop' : input.action === 'unsuspend' ? 'start' : input.action;
			...
			await db.update(applicationBuilds).set({ operationalStatus: input.action === 'suspend' ? 'suspended' : input.action === 'unsuspend' ? 'active' : undefined, suspendedAt: input.action === 'suspend' ? new Date() : input.action === 'unsuspend' ? null : undefined, suspensionReason: input.action === 'suspend' ? input.reason : input.action === 'unsuspend' ? null : undefined, updatedAt: new Date() }).where(eq(applicationBuilds.id, application.buildId));
