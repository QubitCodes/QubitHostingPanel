...
export class OfferController {
	...
	public static async remove(request: Request, slug: string, input: DestructiveActionRequest, metadata: RequestMetadata): Promise<Response> { try { const actor = await authorizeAdmin(request, 'offers.delete', metadata); const [existing] = await db.select().from(offers).where(and(eq(offers.slug, slug), isNull(offers.deletedAt))).limit(1); if (!existing) return resp.failure('Offer not found.', resp.codes.RESOURCE_NOT_FOUND, undefined, null, undefined, 404); if (input.confirmationName !== existing.slug || input.connectedResourceNames.length) { await recordAuditLog({ actorUserId: actor.userId, action: 'offer.delete_rejected', resourceType: 'offer', resourceId: existing.id, metadata: { slug, reason: 'Confirmation mismatch.' }, ipAddress: metadata.ipAddress, userAgent: metadata.userAgent }); return resp.failure('Offer deletion confirmation does not match.', resp.codes.ORDER_CANNOT_BE_PROCESSED, undefined, null, undefined, 422); } const [offer] = await db.update(offers).set({ deletedAt: new Date(), deleteReason: 'Deleted from offer administration.', status: 'archived', updatedAt: new Date() }).where(eq(offers.id, existing.id)).returning(); await recordAuditLog({ actorUserId: actor.userId, action: 'offer.deleted', resourceType: 'offer', resourceId: offer.id, metadata: { slug }, ipAddress: metadata.ipAddress, userAgent: metadata.userAgent }); return resp.success('Offer deleted.'); } catch (error) { return failure(error); } }
}
