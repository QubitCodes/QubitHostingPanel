...
async function ownedDomain(
	...
	const [record] = await db
		.select({
			ownershipId: domainOwnerships.id,
			hostname: domainOwnerships.hostname,
			ownershipStatus: domainOwnerships.status,
		...
		.innerJoin(
			workspaces,
			and(
				eq(workspaces.id, domainOwnerships.workspaceId),
		...
		.innerJoin(
			...
			and(
				...
				eq(customers.userId, actor.userId),
				isNull(customers.deletedAt),
			),
		)
		.where(
			and(
				eq(domainOwnerships.id, ownershipId),
...
async function zoneFor(
	domain: Awaited<ReturnType<typeof ownedDomain>>,
	create = false,
) {
	let [zone] = await db
		.select()
		...
		.where(
			and(
				eq(dnsZones.ownershipId, domain.ownershipId),
				isNull(dnsZones.deletedAt),
			),
		)
		.limit(1);
	if (!zone && create)
		[zone] = await db
			...
			.values({
				...
				ownershipId: domain.ownershipId,
...
export function qualifiedDnsRecordName(name: string, hostname: string): string {
	if (name === '@') return hostname;
	return name.endsWith(`.${hostname}`) ? name : `${name}.${hostname}`;
}

export function customerDnsRecord(record: typeof dnsRecords.$inferSelect) {
	return {
		id: record.id,
		name: record.name,
		type: record.type,
		...
		published:
			Boolean(record.providerRecordId) ||
...
async function publishRecord(
	...
	await db
		.update(dnsRecords)
		.set({ providerRecordId, updatedAt: new Date() })
		.where(and(eq(dnsRecords.id, record.id), eq(dnsRecords.zoneId, zoneId)));
...
export class DnsController {
		...
		try {
			...
			const [records, subdomains] = await Promise.all([
				db
					...
					.from(dnsRecords)
					.where(
						and(eq(dnsRecords.zoneId, zone.id), isNull(dnsRecords.deletedAt)),
					)
				...
				db
					.select({
						...
						domainId: applicationDomains.id,
					...
					.innerJoin(
						...
						and(
							...
							eq(applicationBuilds.workspaceId, domain.workspaceId),
							isNull(applicationBuilds.deletedAt),
						),
					)
					.where(and(isNull(applicationDomains.deletedAt))),
			]);
			return resp.success('DNS configuration retrieved.', {
				domain: {
					id: domain.ownershipId,
					hostname: domain.hostname,
				...
				zone: {
					...
					provisioned: Boolean(zone.providerZoneId),
					delegationVerifiedAt: zone.delegationVerifiedAt,
					lastImportedAt: zone.lastImportedAt,
					lastSynchronizedAt: zone.lastSynchronizedAt,
					lastError: zone.lastError,
				},
				records: records.map(customerDnsRecord),
				subdomains: subdomains
					.filter((item) => item.hostname.endsWith(`.${domain.hostname}`))
					.map(({ applicationName, ...item }) => ({
						...item,
						applicationName: String(applicationName?.name ?? 'Application'),
					})),
			});
		} catch (error) {
			return resp.failure(
				error instanceof Error
					? error.message
					: 'Unable to load DNS configuration.',
	...
	public static async import(
		...
		try {
			...
			if (!zone) throw new Error('Unable to create DNS draft.');
			const result =
				input.source === 'public_scan'
					? await scanPublicDns(domain.hostname)
					: input.source === 'zone_file'
						? parseZoneFile(input.zoneFile!, domain.hostname)
						: await importProviderDns(
								input.source,
								domain.hostname,
								input.apiToken,
							);
			await db.transaction(async (transaction) => {
				for (const record of result.records)
					await transaction
						...
						.onConflictDoNothing();
				await transaction.insert(dnsImportRuns).values({
					zoneId: zone.id,
					source:
						input.source === 'public_scan' || input.source === 'zone_file'
							? 'manual'
							: input.source,
					status: 'succeeded',
					discoveredCount: result.records.length,
			...
			return resp.success(
				'DNS records captured into the review draft.',
				result,
				resp.codes.UPDATED,
			);
		} catch (error) {
	...
	public static async provision(
		...
		try {
			...
			const connectedSubdomains = await db
				.select({
			...
			for (const connected of connectedSubdomains.filter((item) =>
				item.hostname.endsWith(`.${domain.hostname}`),
			))
				await ensureManagedApplicationDns(
					domain.workspaceId,
					connected.id,
					connected.hostname,
				);
			...
			return resp.success(
				zone.providerZoneId
					...
					: 'Managed DNS enabled. Change the registrar nameservers, then refresh delegation.',
				{ nameservers },
				resp.codes.UPDATED,
			);
		} catch (error) {
			return resp.failure(
				error instanceof Error ? error.message : 'Unable to provision DNS.',
				resp.codes.EXTERNAL_SERVICE_ERROR,
				undefined,
				null,
				undefined,
	...
	public static async refresh(
		...
		try {
			...
			if (!zone?.nameservers.length)
				throw new Error('Provision the DNS zone first.');
			const observed = (await resolveNs(domain.hostname))
				.map((value) => value.toLowerCase().replace(/\.$/, ''))
				.sort();
			const expected = zone.nameservers
				.map((value) => value.toLowerCase().replace(/\.$/, ''))
				.sort();
			const active = expected.every((value) => observed.includes(value));
			await db
				.update(dnsZones)
				.set({
					status: active ? 'active' : 'pending_delegation',
					delegationVerifiedAt: active ? new Date() : null,
					lastError: active
						? null
						: `Observed nameservers: ${observed.join(', ') || 'none'}`,
					updatedAt: new Date(),
				})
				.where(eq(dnsZones.id, zone.id));
			return resp.success(
				active
					? 'Nameserver delegation is active.'
					: 'Nameserver delegation is still pending.',
	...
	public static async createRecord(
		...
		try {
			...
			let [record] = await db
				.insert(dnsRecords)
				.values({ zoneId: zone.id, ...input, source: 'user' })
				.returning();
			if (!record) throw new Error('Unable to create DNS record.');
			...
			return resp.success(
				zone.providerZoneId
					? 'DNS record created and published.'
					: 'DNS record created in the draft.',
				customerDnsRecord(record),
				resp.codes.CREATED,
				undefined,
				201,
			);
			...
			return resp.failure(
				error instanceof Error ? error.message : 'Unable to create DNS record.',
	...
	public static async mutateRecord(
		request: Request,
		workspaceId: number,
		ownershipId: string,
		recordId: string,
		...
		try {
			...
			if (record.source === 'platform_managed')
				throw new Error(
			...
			if (!input) {
				...
				await db
					...
					.set({
						deletedAt: new Date(),
						deleteReason: 'Removed by customer.',
						isEnabled: false,
						updatedAt: new Date(),
					})
				...
				return resp.success('DNS record removed.', null, resp.codes.UPDATED);
			}
			...
			if (zone.providerZoneId && record.providerRecordId)
				providerRecordId = await updateAuthoritativeRecord(
					...
					record.providerRecordId,
					{
						...
						name: qualifiedDnsRecordName(updated.name, domain.hostname),
			...
			const [saved] = await db
				.update(dnsRecords)
				.set({ ...input, providerRecordId, updatedAt: new Date() })
				.where(eq(dnsRecords.id, record.id))
				.returning();
			await db
				.update(dnsZones)
				.set({
					...(zone.providerZoneId
						? { lastSynchronizedAt: new Date(), lastError: null }
						: {}),
					updatedAt: new Date(),
				})
				.where(eq(dnsZones.id, zone.id));
			await recordAuditLog({
				action: 'dns.record_updated',
				actorUserId: domain.actorUserId,
				resourceId: record.id,
				resourceType: 'dns_record',
				metadata: {
			...
			return resp.success(
				zone.providerZoneId
					? 'DNS record updated and published.'
					: 'DNS draft record updated.',
				saved ? customerDnsRecord(saved) : null,
				resp.codes.UPDATED,
			);
		} catch (error) {
			return resp.failure(
				error instanceof Error ? error.message : 'Unable to update DNS record.',
				resp.codes.GENERAL_BUSINESS_LOGIC_ERROR,
				undefined,
				null,
				undefined,
