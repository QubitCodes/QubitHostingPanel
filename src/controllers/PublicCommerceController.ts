...
import { entitlementDefinitions, offerEligiblePrices, offerRedemptions, offers, packageCategories, packageEntitlements, packagePrices, packages } from '@db/schema';
...
export class PublicCommerceController {
	...
	public static async quote(input: CheckoutQuoteInput): Promise<Response> {
		try {
			const [price] = await db.select({ id: packagePrices.id, packageId: packages.id, packageSlug: packages.slug, packageName: packages.name, amountMinor: packagePrices.amountMinor, currency: packagePrices.currency, billingInterval: packagePrices.billingInterval, intervalCount: packagePrices.intervalCount, taxBehavior: packagePrices.taxBehavior }).from(packagePrices).innerJoin(packages, eq(packages.id, packagePrices.packageId)).where(and(eq(packagePrices.id, input.priceId), eq(packagePrices.isActive, true), eq(packagePrices.isPublic, true), eq(packages.status, 'published'), isNull(packagePrices.deletedAt), isNull(packages.deletedAt))).limit(1);
			...
			for (const offer of candidates) {
				if (offer.newCustomerOnly) continue;
			...
			let remaining = price.amountMinor; const appliedOffers: Array<{ id: string; name: string; discountMinor: number }> = [];
			for (const offer of applicable) {
				const discount = Math.min(remaining, offer.discountType === 'percentage' ? Math.round(remaining * (offer.percentageBasisPoints ?? 0) / 10_000) : offer.fixedAmountMinor ?? 0);
				if (discount <= 0) continue; appliedOffers.push({ id: offer.id, name: offer.name, discountMinor: discount }); remaining -= discount; if (!offer.stackable) break;
			}
