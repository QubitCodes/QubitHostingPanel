...
const publicFields = { code: databaseClusters.code, name: databaseClusters.name, engine: databaseClusters.engine, engineVersion: databaseClusters.engineVersion, status: databaseClusters.status, providerResourceId: databaseClusters.providerResourceId, internalHost: databaseClusters.internalHost, port: databaseClusters.port, managementHost: databaseClusters.managementHost, managementPort: databaseClusters.managementPort, managementTlsMode: databaseClusters.managementTlsMode, maximumDatabases: databaseClusters.maximumDatabases, limitsMemory: databaseClusters.limitsMemory, limitsCpus: databaseClusters.limitsCpus, backupConfigurationUuid: databaseClusters.backupConfigurationUuid, backupStatus: databaseClusters.backupStatus, lastHealthCheckedAt: databaseClusters.lastHealthCheckedAt, lastHealthError: databaseClusters.lastHealthError, createdAt: databaseClusters.createdAt };
...
export class DatabaseClusterController {
	...
	public static async update(request: Request, code: string, input: UpdateDatabaseClusterInput, metadata: RequestMetadata): Promise<Response> { try { const admin = await authorizeAdmin(request, 'database_clusters.update', metadata); const [row] = await db.update(databaseClusters).set({ ...input, updatedAt: new Date() }).where(and(eq(databaseClusters.code, code), isNull(databaseClusters.deletedAt))).returning(publicFields); if (!row) return resp.failure('Database cluster not found.', resp.codes.RESOURCE_NOT_FOUND, undefined, null, undefined, 404); await recordAuditLog({ actorUserId: admin.userId, action: 'database_cluster.updated', resourceType: 'database_cluster', metadata: { code, changedFields: Object.keys(input) }, ipAddress: metadata.ipAddress, userAgent: metadata.userAgent }); return resp.success('Database cluster updated.', row, resp.codes.UPDATED); } catch (error) { return resp.failure(error instanceof Error ? error.message : 'Permission denied.', resp.codes.PERMISSION_DENIED, undefined, null, undefined, 403); } }
