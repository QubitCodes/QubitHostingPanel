...
export class DatabaseClusterController {
	...
	public static async validate(request: Request, code: string, metadata: RequestMetadata): Promise<Response> { try { await authorizeAdmin(request, 'database_clusters.update', metadata); const [cluster] = await db.select().from(databaseClusters).where(and(eq(databaseClusters.code, code), isNull(databaseClusters.deletedAt))).limit(1); if (!cluster) return resp.failure('Database cluster not found.', resp.codes.RESOURCE_NOT_FOUND, undefined, null, undefined, 404); const remote = await provider.getSharedDatabase(cluster.providerResourceId); const running = remote.status?.toLowerCase().includes('running') ?? false; await db.update(databaseClusters).set({ status: running ? 'active' : 'unavailable', lastHealthCheckedAt: new Date(), lastHealthError: running ? null : `Coolify status: ${remote.status ?? 'unknown'}`, updatedAt: new Date() }).where(eq(databaseClusters.id, cluster.id)); return resp.success('Database cluster validated.', { connected: running, status: remote.status ?? 'unknown' }); } catch (error) { return resp.failure(error instanceof Error ? error.message : 'Validation failed.', resp.codes.EXTERNAL_SERVICE_ERROR, undefined, null, undefined, 502); } }
